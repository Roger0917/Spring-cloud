...
public class HelperUtil {
	...
	/**
	 * 得到config.properties对应的值
	 * 
	 * @return 如无对应值，则返回null
	 */
	...
	public static String getRootAccount() {
		try {
			Properties pro = findByName("config.properties");
			return pro.getProperty("root.account");
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	/**
	 * 得到要同步的平台 key=区域代码 code=区域名称
	 * 
	 * @return
	 */
	public static LinkedHashMap<String, String> getSynPlat() {
		try {
			Properties pro = findByName("config.properties");
			String synPlat = pro.getProperty("syn.plat");
			LinkedHashMap<String, String> map = new LinkedHashMap<>();
			String[] plats = synPlat.split(";");
			if (plats.length > 0) {
	...
	public static boolean checkPhoneNumber(String phoneNumber) {
		...
		if (phoneNumber.length() == 11 && Pattern.compile(mobilePhone).matcher(phoneNumber).find()) {
			return true;
		}
		// -------------带分隔符的座机电话----------------------------------------------
		String phoneWithPrefix = "[0]{1}[0-9]{2,3}-[0-9]{7,8}";
		if (phoneNumber.length() == 12 && Pattern.compile(phoneWithPrefix).matcher(phoneNumber).find()) {
			return true;
		}
		// 不带区号的座机号码
		String Phone = "[0-9]{8}";
		if (phoneNumber.length() == 8 && Pattern.compile(Phone).matcher(phoneNumber).find()) {
			return true;
		}
		return false;
	}
